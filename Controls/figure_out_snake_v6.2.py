...
while(still_not_found):
    ...
    while(True):
        if(y0/req_amplitude_y<=error_margin/100 and z0/req_amplitude_z <=error_margin/100 and abs(x0-wavelength*(n))/(wavelength)<=error_margin/100):
            print("SUCCESS")
...
print("Percentage error: ",error_margin)
...
def update(frame):
    for j, line in enumerate(lines):
        if frame + j < len(answers) - 1:
            x_test = np.linspace(answers[frame + j][0], answers[frame + j + 1][0], 100)
            y_test = np.linspace(answers[frame + j][1], answers[frame + j + 1][1], 100)
            z_test = np.linspace(answers[frame + j][2], answers[frame + j + 1][2], 100)
            line.set_data(x_test, y_test)
            line.set_3d_properties(z_test)
    return lines
# Create line objects for each segment
lines = [ax.plot([], [], [], 'purple' if i < req_rods else colors[i % 3], lw=2)[0] for i in range(req_rods)]

# Setup animation
ani = FuncAnimation(fig, update, frames=range(len(answers) - req_rods), init_func=init, blit=True)
